...
public class OrderDAOViewImp implements OrderDAOView {
	...
	public void cancelorderbyId(Integer productId, int orderId,int custid) {
		...
		String getOrderAmt="select orpr_price from slam_orderProducts where ordr_id=? and prod_id=?";
		String updateWallet="update slam_wallet set amount=? where cust_id=?";
		String getWallemtamt="select amount from slam_wallet where cust_id=?";
		jdbcTemplate.update(updateQuery_products, productId, orderId);
		int wamt=jdbcTemplate.queryForObject(getWallemtamt, Integer.class, custid);
		int amt=jdbcTemplate.queryForObject(getOrderAmt, Integer.class, orderId,productId);
		jdbcTemplate.update(updateWallet, amt+wamt, custid);
